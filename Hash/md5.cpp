...
{
	BYTE bMD5[16];
	ZeroMemory(bMD5, sizeof(bMD5));

	MD5_CTX ctx;
	MD5_Init(&ctx);
	MD5_Update(&ctx, (BYTE*)buffer, len);
	MD5_Final(bMD5, &ctx);

	//*(LONGLONG*)bMD5 ^= MD5_MASK_1;
	//*(LONGLONG*)(bMD5 + 8) ^= MD5_MASK_2;

	for(int i = 0; i < 16; i++)
	{
		wsprintf(&md5[i*2], _T("%02x"), bMD5[i]);
	}
}
...
bool md5_file(_TCHAR* md5, const _TCHAR* filename)
{
	...
	fclose(fp);
	MD5_Final(ctx.digest, &ctx);
	
	TCHAR tmp[33];
	for(int i = 0; i < 16; i++)
	{
		wsprintf(tmp, _T("%02x"), ctx.digest[i]);
		_tcscat(md5, tmp);
	}
	return true;
}