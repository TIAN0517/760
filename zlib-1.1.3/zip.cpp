...
int write_datablock(FILE* fout, linkedlist_data* ll) 
{
    linkedlist_datablock_internal* ldi;
    ldi = ll->first_block;
    while (ldi!=NULL)
...
int ziplocal_putValue(FILE *file, uLong x, int nbByte);
int ziplocal_putValue(FILE *file, uLong x, int nbByte)
{
    unsigned char buf[4];
    int n;
...
void ziplocal_putValue_inmemory(void* dest, uLong x, int nbByte);
...
uLong ziplocal_TmzDateToDosDate(tm_zip* ptm, uLong dosDate)
{
...
zipFile zipOpen (const TCHAR *pathname, int append)
{
    zip_internal ziinit;
    zip_internal* zi;

    ziinit.filezip = _tfopen(pathname, (append == 0) ? _T("wb") : _T("ab"));
...
int zipOpenNewFileInZip (zipFile file, const TCHAR* filename, const zip_fileinfo* zipfi, 
                                        ...
                                        const char* comment, int method, int level)
{
...
#ifdef UNICODE
    int size_needed = WideCharToMultiByte(CP_ACP, 0, filename, _tcslen(filename), NULL, 0, NULL, NULL);
    char* pszFileName = new char[size_needed + 1];
	memset(pszFileName, 0, size_needed + 1);
    WideCharToMultiByte(CP_ACP, 0, filename, _tcslen(filename), pszFileName, size_needed, NULL, NULL);
#else
	pszFileName = (char*)filename;
#endif

    if (comment==NULL)
        size_comment = 0;
    else
        size_comment = lstrlenA(comment);

    size_filename = lstrlenA(pszFileName);
    ...
    else
    {
        ...
        else zi->ci.dosDate = ziplocal_TmzDateToDosDate((tm_zip_s*)&zipfi->tmz_date,zipfi->dosDate);
    }
    ...
    for (i=0;i<size_comment;i++)
        *(zi->ci.central_header+SIZECENTRALHEADER+size_filename+
              size_extrafield_global+i) = *(pszFileName+i);
...
int zipWriteInFileInZip(zipFile file, const voidp buf, unsigned len)
{
    zip_internal* zi;
    int err=ZIP_OK;
